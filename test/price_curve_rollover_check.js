...
async function main() {
  ...
  let b1 = await presale.batchInfo(1);
  console.log("Batch 1 price:", b1[0].toString());
  ...
  let b2 = await presale.batchInfo(2);
  console.log("Batch 2 price:", b2[0].toString());

  if (b1[0] !== 10000n) throw new Error("Batch 1 price wrong");
  if (b2[0] !== 30000n) throw new Error("Batch 2 price wrong");

  console.log("PRICE CURVE ROLLOVER CHECK PASSED");
}
