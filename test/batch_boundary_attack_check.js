...
async function main() {
  console.log("=== BATCH BOUNDARY ATTACK CHECK ===");

  const [owner, attacker, buyer] = await hre.ethers.getSigners();
  ...
  const beforeBatch = await presale.currentBatchId();

  console.log("initial batch:", beforeBatch.toString());
  ...
  const txid1 = hre.ethers.keccak256(
    hre.ethers.toUtf8Bytes("boundary-buy-1")
  );

  await presale.connect(attacker).buyWithUSDT(
    5000n * 1000000n,
    txid1
  );

  const soldBeforeAdvance =
    await presale.totalNominalSold();

  console.log(
    "sold before boundary advance:",
    soldBeforeAdvance.toString()
  );
  ...
  const afterAdvanceBatch =
    await presale.currentBatchId();

  console.log(
    "batch after advance:",
    afterAdvanceBatch.toString()
  );

  const txid2 = hre.ethers.keccak256(
    hre.ethers.toUtf8Bytes("boundary-buy-2")
  );
  ...
  console.log("sold after boundary:", soldAfter.toString());

  if (afterAdvanceBatch <= beforeBatch) {
    throw new Error("batch did not advance");
  }

  if (soldAfter <= soldBeforeAdvance) {
    throw new Error("post-boundary accounting failed");
  }
  ...
  console.log("BATCH BOUNDARY ATTACK CHECK PASSED");
}
