...
async function main() {
  console.log("=== FINALIZE IRREVERSIBILITY INVARIANT CHECK ===");
  ...
  const txid = hre.ethers.keccak256(
    hre.ethers.toUtf8Bytes("finalize-invariant")
  );
  ...
  try {
    const txid2 = hre.ethers.keccak256(
      hre.ethers.toUtf8Bytes("post-finalize-buy")
    );
  ...
  console.log("buyBlocked:", buyBlocked);
  ...
  if (batchAfter !== batchBefore) {
    throw new Error(
      "batch mutated after finalize"
    );
  }

  if (!buyBlocked) {
    throw new Error(
      "buy allowed after finalize"
    );
  }

  if (!advanceBlocked) {
    throw new Error(
      "advance allowed after finalize"
    );
  }

  if (!finalizeReplayBlocked) {
    throw new Error(
      "finalize replay allowed"
    );
  }

  console.log(
    "FINALIZE IRREVERSIBILITY INVARIANT CHECK PASSED"
  );
}
